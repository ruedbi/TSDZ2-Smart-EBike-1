...
#define MOTOR_OVER_SPEED_ERPS                                                                                                                                  \
	((PWM_CYCLES_SECOND / 29) < 650 ? (PWM_CYCLES_SECOND / 29)                                                                                                 \
									: 650)	// motor max speed | 29 points for the sinewave at max speed (less than PWM_CYCLES_SECOND/29)
...
#define CADENCE_TICKS_STARTUP                                                                                                                                  \
	(uint16_t)((uint32_t)PWM_CYCLES_SECOND * 10U / 25U) // ui16_cadence_sensor_ticks value for startup. About 7-8 RPM (6250 at 15.625KHz)
#define CADENCE_SENSOR_STANDARD_MODE_SCHMITT_TRIGGER_THRESHOLD                                                                                                 \
	(uint16_t)((uint32_t)PWM_CYCLES_SECOND * 10U / 446U) // software based Schmitt trigger to stop motor jitter when at resolution limits (350 at 15.625KHz)
...
#define WHEEL_SPEED_SENSOR_TICKS_COUNTER_MIN                                                                                                                   \
	(uint16_t)((uint32_t)PWM_CYCLES_SECOND * 1000U / 477U) // 32767@15625KHz could be a bigger number but will make for a slow detection of stopped wheel speed
...
#define ADC_TORQUE_SENSOR_RANGE_TARGET_MIN                                                                                                                     \
	(uint16_t)(                                                                                                                                                \
		(float)((ADC_TORQUE_SENSOR_RANGE_TARGET / 2)                                                                                                           \
...
#define ADC_TORQUE_SENSOR_DELTA_WITH_WEIGHT                                                                                                                    \
	(uint16_t)(((((ADC_TORQUE_SENSOR_TARGET_WITH_WEIGHT * ADC_TORQUE_SENSOR_RANGE_TARGET_MIN) / ADC_TORQUE_SENSOR_RANGE_TARGET)                                \
				 * (100 + PEDAL_TORQUE_ADC_RANGE_ADJ) / 100)                                                                                                   \
				* (ADC_TORQUE_SENSOR_TARGET_WITH_WEIGHT - ADC_TORQUE_SENSOR_CALIBRATION_OFFSET + ADC_TORQUE_SENSOR_OFFSET_ADJ                                  \
				   - ((ADC_TORQUE_SENSOR_DELTA_ADJ * ADC_TORQUE_SENSOR_TARGET_WITH_WEIGHT) / ADC_TORQUE_SENSOR_RANGE_TARGET)))                                 \
			   / ADC_TORQUE_SENSOR_TARGET_WITH_WEIGHT)

#define PEDAL_TORQUE_PER_10_BIT_ADC_STEP_CALC_X100                                                                                                             \
	(uint8_t)((uint16_t)(((WEIGHT_ON_PEDAL_FOR_STEP_CALIBRATION * 167)                                                                                         \
						  / ((ADC_TORQUE_SENSOR_DELTA_WITH_WEIGHT * ADC_TORQUE_SENSOR_RANGE_TARGET_MAX)                                                        \
							 / (ADC_TORQUE_SENSOR_RANGE_TARGET_MAX                                                                                             \
								- (((ADC_TORQUE_SENSOR_RANGE_TARGET_MAX - ADC_TORQUE_SENSOR_DELTA_WITH_WEIGHT) * 10) / PEDAL_TORQUE_ADC_ANGLE_ADJ)))           \
...
#define BATTERY_VOLTAGE_SHUTDOWN_8_BIT                                                                                                                         \
	(uint8_t)((uint16_t)((BATTERY_LOW_VOLTAGE_CUT_OFF * 250 / BATTERY_VOLTAGE_PER_10_BIT_ADC_STEP_X1000)) - ((uint16_t)DIFFERENCE_CUT_OFF_SHUTDOWN_8_BIT))
