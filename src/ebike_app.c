...
static uint8_t ui8_lights_flag = 0; // ruedbi: used in uart_receive_package() to finally set ui8_lights_state
static uint8_t ui8_lights_on_5s = 0; // ruedbi: overrules ui8_lights_flag
...
static uint8_t ui8_menu_counter = 0; // count up at 100ms at every uart receive
...
static uint8_t ui8_display_data_enabled = 0; // set to 1 if menu-data shall be displayed instead of speed values
...
static uint8_t ui8_lights_button_flag = 0; // set to the current lights status as requested by display
...
// UART
volatile uint8_t ui8_received_package_flag = 0; // 1, if full package has been received
...
static uint8_t ui8_riding_mode_parameter_array[8][5] = {{POWER_ASSIST_LEVEL_OFF, POWER_ASSIST_LEVEL_ECO, POWER_ASSIST_LEVEL_TOUR, POWER_ASSIST_LEVEL_SPORT, POWER_ASSIST_LEVEL_TURBO},
  {TORQUE_ASSIST_LEVEL_0, TORQUE_ASSIST_LEVEL_1, TORQUE_ASSIST_LEVEL_2, TORQUE_ASSIST_LEVEL_3, TORQUE_ASSIST_LEVEL_4}, {CADENCE_ASSIST_LEVEL_0, CADENCE_ASSIST_LEVEL_1, CADENCE_ASSIST_LEVEL_2, CADENCE_ASSIST_LEVEL_3, CADENCE_ASSIST_LEVEL_4},
  {EMTB_ASSIST_LEVEL_0, EMTB_ASSIST_LEVEL_1, EMTB_ASSIST_LEVEL_2, EMTB_ASSIST_LEVEL_3, EMTB_ASSIST_LEVEL_4}, {POWER_ASSIST_LEVEL_OFF, POWER_ASSIST_LEVEL_ECO, POWER_ASSIST_LEVEL_TOUR, POWER_ASSIST_LEVEL_SPORT, POWER_ASSIST_LEVEL_TURBO},
  ...
  {WALK_ASSIST_LEVEL_0, WALK_ASSIST_LEVEL_1, WALK_ASSIST_LEVEL_2, WALK_ASSIST_LEVEL_3, WALK_ASSIST_LEVEL_4}, {0, 0, 0, 0, 0}};
...
// minimum value for these displays
	...
	ui8_duty_cycle_ramp_up_inverse_step_default = map_ui8((uint8_t)MOTOR_ACCELERATION, (uint8_t)0, (uint8_t)100, (uint8_t)PWM_DUTY_CYCLE_RAMP_UP_INVERSE_STEP_DEFAULT, (uint8_t)PWM_DUTY_CYCLE_RAMP_UP_INVERSE_STEP_MIN);

	// set duty cycle ramp down inverse step default
	ui8_duty_cycle_ramp_down_inverse_step_default = map_ui8((uint8_t)MOTOR_DECELERATION, (uint8_t)0, (uint8_t)100, (uint8_t)PWM_DUTY_CYCLE_RAMP_DOWN_INVERSE_STEP_DEFAULT, (uint8_t)PWM_DUTY_CYCLE_RAMP_DOWN_INVERSE_STEP_MIN);

	// Smooth start counter set
	ui8_smooth_start_counter_set = map_ui8((uint8_t)SMOOTH_START_SET_PERCENT, (uint8_t)0, (uint8_t)100, (uint8_t)255, (uint8_t)SMOOTH_START_RAMP_MIN);
...
// enable data displayed on startup
	...
	ui8_adc_battery_current_max_temp_1 = (uint8_t)((uint16_t)(m_configuration_variables.ui8_battery_current_max * 100U) / BATTERY_CURRENT_PER_10_BIT_ADC_STEP_X100);

	// calculate the max adc battery power from the power limit received in offroad mode
	ui32_adc_battery_power_max_x1000_array[OFFROAD_MODE] = (uint32_t)((uint32_t)TARGET_MAX_BATTERY_POWER * 100U * 1000U) / BATTERY_CURRENT_PER_10_BIT_ADC_STEP_X100;

	// calculate the max adc battery power from the received power limit in street mode
	ui32_adc_battery_power_max_x1000_array[STREET_MODE] = (uint32_t)((uint32_t)STREET_MODE_POWER_LIMIT * 100U * 1000U) / BATTERY_CURRENT_PER_10_BIT_ADC_STEP_X100;
...
void ebike_app_controller(void) {
	...
	case 0: uart_receive_package(); break;
	case 1:
		ebike_control_lights();
		calc_oem_wheel_speed();
		break;
	case 2: uart_send_package(); break;
	case 3:
		check_system();
		check_battery_soc();
		break;
...
// field weakening enabled
	...
	if ((ui16_motor_speed_erps > MOTOR_SPEED_FIELD_WEAKENING_MIN) && (ui8_adc_battery_current_filtered < ui8_controller_adc_battery_current_target) && (ui8_adc_throttle_assist == 0U)) {
...
// select optional ADC function
	...
	__asm ld a,
	  0x53eb // ADC1->DB5RL
	  cp a,
	  _ui8_adc_battery_overcurrent jrc 00011 $ mov _ui8_error_battery_overcurrent + 0, #ERROR_BATTERY_OVERCURRENT 00011 $: __endasm;
#endif
	...
	if ((ui8_brake_state) || (!ui8_motor_enabled) || (ui8_system_state == ERROR_MOTOR_BLOCKED) || (ui8_system_state == ERROR_MOTOR_CHECK) || (ui8_system_state == ERROR_BATTERY_OVERCURRENT) || (ui8_system_state == ERROR_THROTTLE) ||
	  (ui8_assist_level == OFF) || (ui8_riding_mode_parameter == 0U) || (ui8_battery_SOC_saved_flag) || ((ui8_system_state != NO_ERROR) && (!m_configuration_variables.ui8_assist_with_error_enabled))) {
	...
	if (ui8_motor_enabled &&
	  ((ui8_system_state == ERROR_MOTOR_BLOCKED) || (ui8_system_state == ERROR_MOTOR_CHECK) || (ui8_system_state == ERROR_BATTERY_OVERCURRENT) || (ui8_system_state == ERROR_THROTTLE) || (ui8_battery_SOC_saved_flag) ||
		((ui16_motor_speed_erps == 0) && (ui8_adc_battery_current_target == 0U) && (ui8_g_duty_cycle == 0U)))) {
		ui8_motor_enabled = 0;
		motor_disable_pwm();
	} else if (!ui8_motor_enabled &&
	  (ui16_motor_speed_erps < ERPS_SPEED_OF_MOTOR_REENABLING) // enable the motor only if it rotates slowly or
	  // is stopped
...
static void apply_startup_boost(void) {
	...
	case CADENCE: ui8_startup_boost_flag = 1; break;
	case SPEED:
...
static void apply_power_assist(void) {
	...
	if (m_configuration_variables.ui8_assist_without_pedal_rotation_enabled) {
		if ((ui8_pedal_cadence_RPM == 0U) && (ui16_adc_pedal_torque_delta > (120 - ui8_assist_without_pedal_rotation_threshold))) {
	...
	if ((ui8_pedal_cadence_RPM > 0U) || (ui8_startup_assist_adc_battery_current_target)) {
		...
		uint32_t ui32_power_assist_x100 = (((uint32_t)(ui8_pedal_cadence_RPM * ui8_power_assist_multiplier_x50)) * ui32_pedal_torque_x100) >> 9; // see note below
...
// smooth start
	...
	if (((ui16_adc_pedal_torque_delta > 0U) && (ui8_pedal_cadence_RPM > 0U)) || (ui8_startup_assist_adc_battery_current_target)) {
		...
		uint16_t ui16_adc_battery_current_target_torque_assist = (ui16_adc_pedal_torque_delta * ui8_torque_assist_factor) / TORQUE_ASSIST_FACTOR_DENOMINATOR;
...
#define eMTB_ASSIST_DENOMINATOR_MIN 10
	...
	if (((ui16_adc_pedal_torque_delta) && (ui8_pedal_cadence_RPM > 0U)) || (ui8_startup_assist_adc_battery_current_target)) {
		...
		uint16_t ui16_eMTB_adc_pedal_torque_delta = (uint16_t)((uint32_t)((ui16_adc_pedal_torque_delta * ui16_adc_pedal_torque_delta) + ui16_eMTB_assist_denominator) / ui16_eMTB_assist_denominator);
...
// smooth start
	...
	if ((ui8_pedal_cadence_RPM > 0U) || (ui8_startup_assist_adc_battery_current_target)) {
		...
		// calculate power assist by multiplying human power with the power assist multiplier
		uint32_t ui32_power_assist_x100 = (((uint32_t)(ui8_pedal_cadence_RPM * ui8_power_assist_multiplier_x50)) * ui16_pedal_torque_x100) >> 9; // see note below
...
#define CRUISE_PID_OUTPUT_LIMIT 1000
	...
	if ((ui16_wheel_speed_x10 >= ui8_cruise_threshold_speed_x10) && (ui8_cruise_assist_flag) && ((ui8_pedal_cadence_RPM > 0U) || (ui8_cruise_button_flag)))
#else
	if ((ui16_wheel_speed_x10 >= ui8_cruise_threshold_speed_x10) && (ui8_cruise_assist_flag) && (ui8_pedal_cadence_RPM > 0U))
#endif
#else
	if ((ui16_wheel_speed_x10 >= ui8_cruise_threshold_speed_x10) && (ui8_cruise_assist_flag) && (ui8_pedal_cadence_RPM > 0U))
#endif
...
#if CRUISE_MODE_WALK_ENABLED
#if ENABLE_BRAKE_SENSOR
	if ((ui16_wheel_speed_x10 >= ui8_cruise_threshold_speed_x10) && (!m_configuration_variables.ui8_street_mode_enabled) && (ui8_cruise_assist_flag) && ((ui8_pedal_cadence_RPM > 0U) || (ui8_cruise_button_flag)))
#else
	if ((ui16_wheel_speed_x10 >= ui8_cruise_threshold_speed_x10) && (!m_configuration_variables.ui8_street_mode_enabled) && (ui8_cruise_assist_flag) && (ui8_pedal_cadence_RPM > 0U))
#endif
#else
	if ((ui16_wheel_speed_x10 >= ui8_cruise_threshold_speed_x10) && (!m_configuration_variables.ui8_street_mode_enabled) && (ui8_cruise_assist_flag) && (ui8_pedal_cadence_RPM))
#endif
#endif
	{
		...
		i16_control_output = (CRUISE_PID_KP * i16_error) + ((CRUISE_PID_KI_X16 * i16_integral) >> 4) + (CRUISE_PID_KD * i16_derivative);
...
static void apply_throttle(void) {
	...
	ui8_throttle_adc_in = map_ui8((uint8_t)(ui16_adc_throttle >> 2), (uint8_t)ADC_THROTTLE_MIN_VALUE, (uint8_t)ADC_THROTTLE_MAX_VALUE, (uint8_t)0, (uint8_t)255);
	...
	case W_O_P_6KM_H_AND_PEDALING:
		if ((ui16_wheel_speed_x10 > WALK_ASSIST_THRESHOLD_SPEED_X10) && (ui8_pedal_cadence_RPM == 0U)) {
			ui8_adc_throttle_assist = 0;
		}
		break;
	default: break;
	...
	uint8_t ui8_adc_battery_current_target_throttle = map_ui8((uint8_t)ui8_adc_throttle_assist, (uint8_t)0, (uint8_t)255, (uint8_t)0, (uint8_t)ui8_adc_battery_current_max);
	...
	if (ui8_adc_battery_current_target_throttle > ui8_adc_battery_current_target) {
		...
		if (ui16_wheel_speed_x10 >= 255) {
			...
			ui8_duty_cycle_ramp_up_inverse_step = map_ui8((uint8_t)ui16_wheel_speed_x10, (uint8_t)40, (uint8_t)255, (uint8_t)THROTTLE_DUTY_CYCLE_RAMP_UP_INVERSE_STEP_DEFAULT, (uint8_t)THROTTLE_DUTY_CYCLE_RAMP_UP_INVERSE_STEP_MIN);

			ui8_duty_cycle_ramp_down_inverse_step = map_ui8((uint8_t)ui16_wheel_speed_x10, (uint8_t)40, (uint8_t)255, (uint8_t)PWM_DUTY_CYCLE_RAMP_DOWN_INVERSE_STEP_DEFAULT, (uint8_t)PWM_DUTY_CYCLE_RAMP_DOWN_INVERSE_STEP_MIN);
		}
...
static void apply_speed_limit(void) {
	if (m_configuration_variables.ui8_wheel_speed_max > 0U) {
		...
		// set battery current target
		ui8_adc_battery_current_target = (uint8_t)map_ui16(ui16_wheel_speed_x10, speed_limit_low, speed_limit_high, ui8_adc_battery_current_target, 0U);
...
static void calc_cadence(void) {
	...
	ui16_cadence_ticks_count_min_speed_adj = map_ui16(ui16_wheel_speed_x10, 40, 400, CADENCE_SENSOR_CALC_COUNTER_MIN, CADENCE_SENSOR_TICKS_COUNTER_MIN_AT_SPEED);
...
#define TOFFSET_CYCLES 120 // 3sec (25ms*120)
	...
	if (toffset_cycle_counter < TOFFSET_CYCLES) {
		...
		if ((toffset_cycle_counter == TOFFSET_CYCLES) && (ui8_torque_sensor_calibrated)) {
			if ((ui16_adc_pedal_torque_offset_init > ui16_adc_pedal_torque_offset_min) && (ui16_adc_pedal_torque_offset_init < ui16_adc_pedal_torque_offset_max)) {
...
// calculate coaster brake threshold
	...
	if (ui16_adc_pedal_torque > ui16_adc_pedal_torque_offset) {
		...
		if ((ui8_torque_sensor_calibrated) && (m_configuration_variables.ui8_torque_sensor_adv_enabled)) {
			...
			ui16_adc_pedal_torque_delta = ui16_adc_pedal_torque - ui16_adc_pedal_torque_offset - ((ADC_TORQUE_SENSOR_DELTA_ADJ * ui16_temp) / ADC_TORQUE_SENSOR_RANGE_TARGET);

			// adc pedal torque range adjusment
			ui16_temp = (ui16_adc_pedal_torque_delta * ADC_TORQUE_SENSOR_RANGE_INGREASE_X100) / 10;
			ui16_adc_pedal_torque_delta = (((ui16_temp * ((ui16_temp / ADC_TORQUE_SENSOR_ANGLE_COEFF + ADC_TORQUE_SENSOR_ANGLE_COEFF_X10) / ADC_TORQUE_SENSOR_ANGLE_COEFF)) / 50) // 100
			...
			if (ui16_adc_pedal_torque_delta < ADC_TORQUE_SENSOR_RANGE_TARGET_MAX) {
				ui16_temp = (ui16_adc_pedal_torque_delta * ADC_TORQUE_SENSOR_RANGE_TARGET_MAX) / (ADC_TORQUE_SENSOR_RANGE_TARGET_MAX - (((ADC_TORQUE_SENSOR_RANGE_TARGET_MAX - ui16_adc_pedal_torque_delta) * 10) / PEDAL_TORQUE_ADC_ANGLE_ADJ));
 ...
 */
struct_configuration_variables* get_configuration_variables(void) { return (struct_configuration_variables*)&m_configuration_variables; }
...
#define MOTOR_CHECK_ERPS_THRESHOLD 20 // 20 ERPS
	...
	if (((m_configuration_variables.ui8_riding_mode == POWER_ASSIST_MODE) || (m_configuration_variables.ui8_riding_mode == TORQUE_ASSIST_MODE) || (m_configuration_variables.ui8_riding_mode == HYBRID_ASSIST_MODE) ||
	...
	if ((ui16_motor_speed_erps > MOTOR_CHECK_ERPS_THRESHOLD) && ((ui8_riding_torque_mode) || (m_configuration_variables.ui8_riding_mode == CADENCE_ASSIST_MODE)) && (ui8_adc_battery_current_target == 0U || ui8_duty_cycle_target == 0U)) {
	...
	if (ui8_riding_torque_mode) {
		if ((ui16_adc_pedal_torque_offset > 300) || (ui16_adc_pedal_torque_offset < 10) || (ui16_adc_pedal_torque > 500) || (ui8_adc_pedal_torque_offset_error)) {
...
void UART2_IRQHandler(void) __interrupt(UART2_IRQHANDLER) {
	...
	if ((UART2->SR & (uint8_t)(UART2_FLAG_OR_LHE | UART2_FLAG_NF | UART2_FLAG_FE)) != (uint8_t)0x00) {
		// There is an error
		// Clear error bits
		ui8_byte_received = (uint8_t)UART2->DR; // According to datasheet : The Error bits (OR, NF, FE) are
...
static void uart_receive_package(void) {
	...
	if (ui8_received_package_flag) {
		...
		if (ui8_rx_check_code == ui8_rx_buffer[RX_CHECK_CODE]) {
			...
			case ASSIST_PEDAL_LEVEL01: ui8_assist_level = ECO; break;
			...
			if (ui8_lights_button_flag) {
				...
				if ((!ui8_lights_flag) && ((m_configuration_variables.ui8_set_parameter_enabled) || (ui8_assist_level == OFF) || ((ui8_startup_counter < DELAY_MENU_ON) && (ui8_assist_level == TURBO)))) {
					// ruedbi: this is the action when lights have been turned on
					...
					if (!ui8_menu_flag) {
						...
						// rbien: ... from the *_temp variables
					...
					if ((ui8_lights_counter >= DELAY_LIGHTS_ON) || ((ui8_assist_level != ui8_assist_level_temp) && (!ui8_torque_sensor_calibration_flag) && (!ui8_auto_display_data_flag))) {
						// set lights flag
						ui8_lights_flag = 1;
						// lights 5s off
						ui8_lights_on_5s = 0;
			...
			if ((ui8_menu_counter >= ui8_delay_display_function) || ((ui8_assist_level != ui8_assist_level_temp) && (!ui8_torque_sensor_calibration_flag) && (!ui8_auto_display_data_flag))) {
				// clear menu flag
				ui8_menu_flag = 0;
				// clear menu index
				ui8_menu_index = 0;
			...
			if (ui8_menu_function_enabled) {
				...
				if ((m_configuration_variables.ui8_set_parameter_enabled) || (ui8_assist_level == OFF) || ((ui8_startup_counter < DELAY_MENU_ON) && (ui8_assist_level == TURBO))) {
					switch (ui8_assist_level) {
					case OFF:
						// set parameter
						switch (ui8_menu_index) {
...
#if STARTUP_ASSIST_ENABLED
				// walk assist button pressed
				if ((ui8_walk_assist_button_pressed) && (ui8_display_ready_flag) && (!ui8_walk_assist_flag) && (ui8_lights_flag)) {
...
// walk assist mode
				...
				// walk assist button pressed
				if ((ui8_walk_assist_button_pressed) && (ui8_display_ready_flag) && (!ui8_startup_assist_flag) && (ui8_walk_assist_enabled_array[m_configuration_variables.ui8_street_mode_enabled])) {
					...
					if (ui8_walk_assist_flag) {
						...
						if (ui8_walk_assist_debounce_counter < WALK_ASSIST_DEBOUNCE_TIME) {
							...
							if ((ui8_assist_level != ui8_walk_assist_level) || (ui8_brake_state) || (m_configuration_variables.ui8_street_mode_enabled)) {
...
// set lights
			...
			if ((m_configuration_variables.ui8_riding_mode == WALK_ASSIST_MODE) || (ui8_startup_assist_flag)) {
				m_configuration_variables.ui8_wheel_speed_max = WALK_ASSIST_THRESHOLD_SPEED;
			} else if ((ui8_throttle_mode_array[m_configuration_variables.ui8_street_mode_enabled] == W_O_P_6KM_H_ONLY) && (ui8_throttle_adc_in)) {
				m_configuration_variables.ui8_wheel_speed_max = WALK_ASSIST_THRESHOLD_SPEED;
			} else {
			...
			ui8_adc_battery_current_max_temp_2 = (uint8_t)((uint32_t)(ui32_adc_battery_power_max_x1000_array[m_configuration_variables.ui8_street_mode_enabled] / ui16_battery_voltage_filtered_x1000));
...
// reserved for VLCD5, torque sensor value TE and TE1
		...
		// battery power filtered x 10 for display data
		ui16_battery_power_filtered_x10 = filter(ui16_battery_power_x10, ui16_battery_power_filtered_x10, 10);
		ui8_tx_buffer[4] = (uint8_t)(ui16_battery_power_filtered_x10 / 100);
#else
...
// E06 ERROR_OVERTEMPERATURE
		...
		if ((ui8_display_fault_code != NO_FAULT) && (ui8_display_function_code == NO_FUNCTION)) {
			// ruedbi: This is the handling of real errors, displayed as error codes on the display
#if ENABLE_XH18
			// from the manual:
  ...
  // fault code
			...
			// potentially interleaved with data values
			// function code
#if ENABLE_EKD01
			...
			switch (ui8_menu_index) {
			case 1: ui8_display_function_code = 1; break;
			case 2: ui8_display_function_code = 2; break;
			case 3: ui8_display_function_code = 4; break;
			}
#endif
			if ((!ui8_menu_flag) && (ui8_menu_index > 0U) && ((m_configuration_variables.ui8_set_parameter_enabled) || (ui8_assist_level == OFF) || ((ui8_startup_counter < DELAY_MENU_ON) && (ui8_assist_level == TURBO)))) {
				// display a blinking function code
#if 0
				if (ui8_default_flash_state) {
			...
			if (ui8_display_data_enabled) {
				...
				ui16_display_data = ui16_display_data_factor / ui16_battery_SOC_percentage_x10;
					...
					// ruedbi: this here is display_data_on_startup operation mode / non-config mode:
					...
					case 2:
						...
						break;
					default: ui16_display_data = 0; break;
					}
				...
				} else if ((ui8_menu_counter <= ui8_delay_display_function) && (ui8_menu_index > 0U) && ((ui8_assist_level < TOUR) || (ui8_display_alternative_lights_configuration))) { // OFF & ECO &
					// alternative lights
					// configuration
					...
					switch (index_temp) {
					case 0: ui16_display_data = ui16_display_data_factor / FUNCTION_STATUS_OFF; break;
					case 1: ui16_display_data = ui16_display_data_factor / FUNCTION_STATUS_ON; break;
					default: break;
					}
					...
					case 2:
						...
						ui16_display_data = ui16_display_data_factor / ui16_battery_voltage_calibrated_x10;
						break;
					case 3: ui16_display_data = ui16_display_data_factor / ui8_battery_current_filtered_x10; break;
					case 4:
						// battery power filtered x 10 for display data
						ui16_battery_power_filtered_x10 = filter(ui16_battery_power_x10, ui16_battery_power_filtered_x10, 8);
#if UNITS_TYPE == MILES
...
#endif
						break;
					case 5: ui16_display_data = ui16_display_data_factor / (ui16_adc_throttle >> 2); break;
					case 6: ui16_display_data = ui16_display_data_factor / ui16_adc_torque; break;
					case 7:
#if UNITS_TYPE == MILES
					...
					case 8:
						...
						break;
					case 9: ui16_display_data = ui16_display_data_factor / ui16_adc_pedal_torque_delta; break;
					case 10:
#if UNITS_TYPE == MILES
...
#endif
						break;
					case 11: ui16_display_data = ui16_display_data_factor / ui16_motor_speed_erps; break;
					case 12:
...
// set working flag
	...
	static void calc_oem_wheel_speed(void) {
		if (ui8_display_ready_flag) {
			...
			ui16_oem_wheel_speed_time =
			  (uint16_t)((uint32_t)(uint16_t)(ui8_oem_wheel_diameter * 80U * 10U) * ui16_wheel_speed_sensor_ticks / ((uint32_t)m_configuration_variables.ui16_wheel_perimeter * OEM_WHEEL_SPEED_DIVISOR)); // OEM_WHEEL_SPEED_DIVISOR is x10
		}
...
//ui8_battery_state_of_charge = 0; // don't care
	...
	if (ui16_battery_voltage_soc_filtered_x10 > BATTERY_SOC_VOLTS_8_X10) {
		...
		if ((ui8_display_ready_flag)) {
			...
			if ((ui8_battery_SOC_init_flag) && (!ui8_battery_SOC_reset_flag) && (ui8_startup_counter++ > DELAY_MENU_ON)) {
				...
				if (m_configuration_variables.ui8_soc_percent_calculation == SOC_CALC_AUTO) {
					...
					if ((ui16_actual_battery_SOC_x10 < (ui16_battery_SOC_percentage_x10 - ui8_battery_soc_auto_reset_low_x10)) || (ui16_actual_battery_SOC_x10 > (ui16_battery_SOC_percentage_x10 + BATTERY_SOC_PERCENT_THRESHOLD_X10))) {
