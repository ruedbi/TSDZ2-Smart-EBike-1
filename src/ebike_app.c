...
// select optional ADC function
	...
	else if (!ui8_motor_enabled && (ui16_motor_speed_erps < ERPS_SPEED_OF_MOTOR_REENABLING) // enable the motor only if it rotates slowly or is stopped
...
void UART2_IRQHandler(void) __interrupt(UART2_IRQHANDLER) {
	...
	if ((UART2->SR & (uint8_t)(UART2_FLAG_OR_LHE | UART2_FLAG_NF | UART2_FLAG_FE)) != (uint8_t)0x00) {
		// There is an error
		// Clear error bits
		ui8_byte_received = (uint8_t)UART2
								->DR; // According to datasheet : The Error bits (OR, NF, FE) are reset by a read to the UART_SR register followed by a UART_DR
